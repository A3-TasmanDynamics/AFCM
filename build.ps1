<#
    ...
    ready-to-use @AFCM mod folder next to this project for local/Eden testing -- `hemtt release`
    on its own only produces zip archives under releases\.
    ...
    projects) because this repo has exactly one addon prefix (afcm) and no external Workshop
    dependencies that need de-rapping into the build.

    Usage:
        .\build.ps1                # unsigned build, refreshes @AFCM next to the project
...
$root = $PSScriptRoot
$modFolder = Join-Path $root "@AFCM"

$hemttArgs = @("release")
if (-not $Sign) { $hemttArgs += "--no-sign" }
if ($NoArchive) { $hemttArgs += "--no-archive" }

if ($Sign -and -not (Get-ChildItem -Path $root -Filter "*.biprivatekey" -ErrorAction SilentlyContinue)) {
